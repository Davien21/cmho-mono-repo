...
export const getStockEntriesSchema = paginationQuerySchema.shape({
  ...
  operationType: yup.string().optional().oneOf(['add', 'reduce']).label('Operation type'),
});

export type GetStockEntriesQuerySchema = yup.InferType<typeof getStockEntriesSchema>;
...
export const createStockEntrySchema = yup
  .object<StockEntryRequest>({
    ...
    operationType: yup.string().required().oneOf(['add', 'reduce']).label('Operation type'),
    supplier: yup
      .object({
    ...
    expiryDate: yup
      ...
      .label('Expiry date'),
    quantityInBaseUnits: yup.number().required().label('Quantity in base units'),
  })
  .required();
