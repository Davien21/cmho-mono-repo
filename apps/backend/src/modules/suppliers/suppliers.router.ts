...
router.post(
  '/inventory/suppliers',
  [authenticate, hasRole(AdminRole.INVENTORY_MANAGER), validator(createSupplierSchema)],
  createSupplier
);

router.put(
  '/inventory/suppliers/:id',
  [authenticate, hasRole(AdminRole.INVENTORY_MANAGER), validator(updateSupplierSchema)],
  updateSupplier
);
