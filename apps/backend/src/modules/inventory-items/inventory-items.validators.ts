...
export type GetInventoryItemsQuerySchema = yup.InferType<typeof getInventoryItemsSchema>;
...
export const createInventoryItemSchema = yup
  .object<IInventoryItemRequest>({
    ...
    status: yup
      ...
      .required()
      .label('Status'),
    currentStockInBaseUnits: yup.number().optional().label('Current stock in base units'),
  })
  .required();

export const updateInventoryItemSchema = yup.object<Partial<IInventoryItemRequest>>({
  ...
  setupStatus: yup.mixed<InventorySetupStatus>().oneOf(['draft', 'ready']).label('Setup status'),
  status: yup.mixed<InventoryStatus>().oneOf(['active', 'disabled', 'deleted']).label('Status'),
